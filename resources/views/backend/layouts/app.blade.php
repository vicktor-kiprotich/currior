...
<head>
    ...
    <meta charset="utf-8" />
    <link rel="icon"
        href="@if (get_setting('site_icon')) {{ uploaded_asset(get_setting('site_icon')) }} @else {{ static_asset('assets/dashboard/media/logos/favicon.ico') }} @endif">
    @if (get_setting('site_name'))
        <title>
            @if (View::hasSection('sub_title'))
                @yield('sub_title') |
            @endif {{ get_setting('site_name') }}
        </title>
    @else
        <title>
            @if (View::hasSection('sub_title'))
                @yield('sub_title') |
            @endif {{ translate('FS Express') }}
        </title>
    @endif
    ...
    @if (\App\Language::where('code', Session::get('locale', Config::get('app.locale')))->first()->rtl == 1)
        ...
        <link href="{{ static_asset('assets/dashboard/plugins/global/plugins.bundle.rtl.css') }}" rel="stylesheet"
            type="text/css" />
        <link href="{{ static_asset('assets/dashboard/plugins/custom/prismjs/prismjs.bundle.rtl.css') }}"
            rel="stylesheet" type="text/css" />
        <link href="{{ static_asset('assets/dashboard/css/style.bundle.rtl.css') }}" rel="stylesheet"
            type="text/css" />
        <!--end::Global Theme Styles-->

        <!--begin::Layout Themes(used by all pages)-->
        <link href="{{ static_asset('assets/dashboard/css/themes/layout/header/base/light.rtl.css') }}"
            rel="stylesheet" type="text/css" />
        <link href="{{ static_asset('assets/dashboard/css/themes/layout/header/menu/light.rtl.css') }}"
            rel="stylesheet" type="text/css" />
...
<body id="kt_body"
    ...
    <div class="d-flex flex-column flex-root">
        ...
        <div class="d-flex flex-column-fluid page flex-row">
            ...
            <div class="d-flex flex-column flex-row-fluid wrapper" id="kt_wrapper">
                ...
                <!--begin::Footer-->
                <div class="footer d-flex flex-lg-column bg-white py-4" id="kt_footer">
                    ...
                    <div class="container text-center">
                        ...
                        <div class="text-dark order-md-1 order-2">
                            <span class="text-muted font-weight-bold mr-1">{{ translate('Copyright') }} &copy;
                                {{ date('Y') }}</span>
                            <a href="http://keenthemes.com/metronic" target="_blank"
                                class="text-dark-75 text-hover-primary">{{ translate('FS Express') }}</a> <span
                                class="text-muted font-weight-bold mr-2">{{ translate('v') }}{{ get_setting('current_version') }}</span>
                        </div>
    ...
    @if (env('GOOGLE_MAPS_API_TOKEN') && \App\BusinessSetting::where('type', 'google_maps')->first()->value == 1)
        <script
