...
@if (
    $user_type == 'admin' ||
        in_array('1101', $staff_permission) ||
        in_array('1007', $staff_permission) ||
        in_array('1108', $staff_permission))
                        ...
                        <table class="aiz-table mb-0 table">
                            ...
                            <tbody>
                                ...
                                @php
                                    ...
                                    $shipments = App\Shipment::limit($count)
                                        ->orderBy('id', 'desc')
                                        ->get();
                                @endphp
                                @foreach ($shipments as $key => $shipment)
                                    <tr>
                                        ...
                                        <td width="5%"><a
                                                href="{{ route('admin.shipments.show', $shipment->id) }}">{{ $shipment->barcode }}</a>
                                        </td>
    ...
    @if ($user_type == 'admin' || in_array('1100', $staff_permission) || in_array('1007', $staff_permission))
        ...
        @php
            $captains = App\Captain::withCount([
                            ...
                            <tbody>
                                @foreach ($captains as $key => $captain)
                                    ...
                                    @if ($captain->wallet > 0 ?? 0)
                                        <tr>
                                            <td><a
                                                    href="{{ route('admin.captains.show', $captain->id) }}">{{ $captain->code }}</a>
                                            </td>
                                            <td>{{ $captain->name }}</td>
                        ...
                        <table class="aiz-table mb-0 table">
                            ...
                            <tbody>
                                ...
                                @foreach ($captains as $key => $captain)
                                    @php
                                        $count = App\ShipmentSetting::getVal('latest_shipment_count') ? App\ShipmentSetting::getVal('latest_shipment_count') : 10;
                                        $shipments = App\Shipment::where('captain_id', $captain->id)
                                            ->limit($count)
                                            ->orderBy('id', 'desc')
                                            ->get();
                                    @endphp
...
@elseif($user_type == 'branch')
                ...
                <div class="card-body">
                    ...
                    <table class="aiz-table mb-0 table">
                        ...
                        <tbody>
                            ...
                            @php
                                $count = App\ShipmentSetting::getVal('latest_shipment_count') ? App\ShipmentSetting::getVal('latest_shipment_count') : 10;
                                $shipments = App\Shipment::where('branch_id', Auth::user()->userBranch->branch_id)
                                    ->limit($count)
                                    ->orderBy('id', 'desc')
                                    ->get();
                            @endphp
    ...
    @php
        $captains = App\Captain::where('branch_id', Auth::user()->id)
            ->withCount([
                'transaction AS wallet' => function ($query) {
                    $query->select(DB::raw('SUM(value)'));
                },
                    ...
                    <table class="aiz-table mb-0 table">
                        ...
                        <tbody>
                            ...
                            @php
                                ...
                                $shipments = App\Shipment::limit($count)
                                    ->orderBy('id', 'desc')
                                    ->where('client_id', Auth::user()->userClient->client_id)
                                    ->get();
                            @endphp
                            @foreach ($shipments as $key => $shipment)
                                <tr>
        ...
        <div class="col-md-12">
            <div class="card card-custom card-stretch">
                ...
                <div class="card-body">
                    ...
                    @php
                        $missions = App\Mission::where('captain_id', Auth::user()->userCaptain->captain_id)
                            ->whereNotIn('status_id', [\App\Mission::DONE_STATUS, \App\Mission::CLOSED_STATUS])
                ...
                <div class="card-body">
                    ...
                    <table class="aiz-table mb-0 table">
                        ...
                        <tbody>
                            ...
                            @php
                                $count = App\ShipmentSetting::getVal('latest_shipment_count') ? App\ShipmentSetting::getVal('latest_shipment_count') : 10;
                                $missions = App\Mission::limit($count)
                                    ->orderBy('id', 'desc')
                                    ->where('captain_id', Auth::user()->userCaptain->captain_id)
                                    ->whereNotIn('status_id', [\App\Mission::DONE_STATUS, \App\Mission::CLOSED_STATUS])
                                    ->where('due_date', \Carbon\Carbon::today()->format('Y-m-d'))
                                    ->get();
                            @endphp
                            @foreach ($missions as $key => $mission)
                                <tr>
                                    <td width="5%"><a
                                            href="{{ route('admin.missions.show', $mission->id) }}">{{ $mission->code }}</a>
                                    </td>
