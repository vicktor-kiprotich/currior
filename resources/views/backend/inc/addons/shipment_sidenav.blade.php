...
@if ($addon != null)
    @if ($addon->activated)
        @if (in_array($user_type, ['admin', 'customer', 'branch']) ||
                in_array('1108', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.shipments.index', 'admin.shipments.update', 'admin.shipments.import', 'admin.shipments.add.by.api', 'admin.shipments.create', 'admin.shipments.show']) }} @foreach (\App\Shipment::status_info() as $item) {{ areActiveRoutes([$item['route_name']]) }} @endforeach"
                ...
                <div class="menu-submenu">
                    ...
                    <ul class="menu-subnav">
                        ...
                        @if (in_array($user_type, ['admin', 'customer', 'captain', 'branch']) ||
                                in_array('1108', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                            <li class="menu-item {{ areActiveRoutes(['admin.shipments.create']) }}"
                                aria-haspopup="true">
                            ...
                            @if (in_array($user_type, ['admin']))
                                <li class="menu-item {{ areActiveRoutes(['admin.shipments.import']) }}"
                                    aria-haspopup="true">
                        ...
                        @if (in_array($user_type, ['admin', 'customer']) ||
                                in_array('1107', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                            <li class="menu-item {{ areActiveRoutes(['admin.shipments.add.by.api']) }}"
                                aria-haspopup="true">
                        ...
                        @if (in_array($user_type, ['admin', 'captain']) ||
                                in_array('1109', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                            <li class="menu-item {{ areActiveRoutes(['admin.shipments.barcode.scanner']) }}"
                                aria-haspopup="true">
                        ...
                        @if (in_array($user_type, ['admin', 'customer', 'captain', 'branch']) ||
                                in_array('1108', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                            <li class="menu-item {{ areActiveRoutes(['admin.shipments.index', 'admin.shipments.show']) }}"
                                aria-haspopup="true">
                        ...
                        @foreach (\App\Shipment::status_info() as $item)
                            @if (in_array($user_type, ['admin', 'customer', 'captain', 'branch']) ||
                                    in_array($item['permissions'], json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                                @if ($item['status'] == \App\Shipment::SAVED_STATUS)
                                    <li class="menu-item @if (isset($type) && $type == \App\Shipment::PICKUP && isset($status) && $status == $item['status']) menu-item-active menu-item-open @endif"
                                        aria-haspopup="true">
                                        <a href="{{ route($item['route_name'], ['status' => $item['status'], 'type' => \App\Shipment::PICKUP]) }}"
                                            class="menu-link">
                                            <i class="menu-bullet menu-bullet-dot">
                                                <span></span>
                                            </i>
                                    ...
                                    @if ($user_type != 'captain')
                                        <li class="menu-item @if (isset($type) && $type == \App\Shipment::DROPOFF && isset($status) && $status == $item['status']) menu-item-active menu-item-open @endif"
                                            aria-haspopup="true">
                                            <a href="{{ route($item['route_name'], ['status' => $item['status'], 'type' => \App\Shipment::DROPOFF]) }}"
                                                class="menu-link">
                                                <i class="menu-bullet menu-bullet-dot">
                                                    <span></span>
                                                </i>
                                ...
                                @else
                                    <li class="menu-item {{ areActiveRoutes([$item['route_name']]) }}"
                                        aria-haspopup="true">
...
@if ($addon != null)
    @if ($addon->activated)
        @if (in_array($user_type, ['admin', 'captain', 'branch']) ||
                in_array('1008', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.missions.index', 'admin.missions.update', 'admin.missions.create', 'admin.missions.show', 'admin.missions.get.manifest']) }} @foreach (\App\Mission::status_info() as $item) {{ areActiveRoutes([$item['route_name']]) }} @endforeach"
                aria-haspopup="true" data-menu-toggle="hover">
                <a href="javascript:;" class="menu-link menu-toggle">
                ...
                <div class="menu-submenu">
                    ...
                    <ul class="menu-subnav">
                        ...
                        @foreach (\App\Mission::status_info() as $item)
                            @if (in_array($user_type, $item['user_role']) ||
                                    in_array($item['permissions'], json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                                <li class="menu-item {{ areActiveRoutes([$item['route_name']]) }}"
                                    aria-haspopup="true">
                                    <a href="{{ route($item['route_name'], ['status' => $item['status']]) }}"
        ...
        @if (in_array($user_type, ['admin', 'captain', 'branch']) ||
                in_array('1008', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
            <li class="menu-item {{ areActiveRoutes(['admin.missions.manifests', 'admin.missions.get.manifest']) }}"
                aria-haspopup="true">
...
@if ($addon != null)
    @if ($addon->activated)
        ...
        <!--Users-->
        @if (in_array($user_type, ['admin', 'branch']) ||
                in_array('1005', json_decode(Auth::user()->staff->role->permissions ?? '[]')) ||
                in_array('1006', json_decode(Auth::user()->staff->role->permissions ?? '[]')) ||
                in_array('1007', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.clients.index', 'admin.clients.update', 'admin.clients.create', 'admin.clients.show']) }}"
                aria-haspopup="true" data-menu-toggle="hover">
                <a href="javascript:;" class="menu-link menu-toggle">
                ...
                <div class="menu-submenu">
                    ...
                    <ul class="menu-subnav">
                            ...
                            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.clients.index', 'admin.clients.update', 'admin.clients.create', 'admin.clients.show']) }}"
                                ...
                                <div class="menu-submenu">
                                    ...
                                    <ul class="menu-subnav">
                                        ...
                                        @if (in_array($user_type, ['admin', 'branch']) ||
                                                in_array('1005', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                                            <li class="menu-item {{ areActiveRoutes(['admin.clients.index', 'admin.clients.update', 'admin.clients.show']) }}"
                                                aria-haspopup="true">
                                            ...
                                            <li class="menu-item {{ areActiveRoutes(['admin.clients.create']) }}"
                                                aria-haspopup="true">
                        ...
                        @if (Auth::user()->user_type == 'admin' || in_array('1006', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.branchs.index', 'admin.branchs.update', 'admin.branchs.create', 'admin.branchs.show']) }}"
                                aria-haspopup="true" data-menu-toggle="hover">
                                <a href="javascript:;" class="menu-link menu-toggle">
                                ...
                                <div class="menu-submenu">
                                    ...
                                    <ul class="menu-subnav">
                                        ...
                                        @if (Auth::user()->user_type == 'admin' || in_array('1006', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                                            <li class="menu-item {{ areActiveRoutes(['admin.branchs.index', 'admin.branchs.update', 'admin.branchs.show']) }}"
                                                aria-haspopup="true">
                                            ...
                                            <li class="menu-item {{ areActiveRoutes(['admin.branchs.create']) }}"
                                                aria-haspopup="true">
                        ...
                        @if (in_array($user_type, ['admin', 'branch']) ||
                                in_array('1007', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.captains.index', 'admin.captains.update', 'admin.captains.create']) }}"
                                aria-haspopup="true" data-menu-toggle="hover">
                                <a href="javascript:;" class="menu-link menu-toggle">
                                    <i class="menu-icon fas fa-people-carry"></i>
                                ...
                                <div class="menu-submenu">
                                    ...
                                    <ul class="menu-subnav">
                                        ...
                                        @if (in_array($user_type, ['admin', 'branch']) ||
                                                in_array('1007', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
                                            <li class="menu-item {{ areActiveRoutes(['admin.captains.index', 'admin.captains.update', 'admin.captains.create']) }}"
                                                aria-haspopup="true">
                                            ...
                                            <li class="menu-item {{ areActiveRoutes(['admin.captains.create']) }}"
                                                aria-haspopup="true">
        ...
        @if (in_array($user_type, ['admin', 'customer', 'branch']) ||
            ...
            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.shipments.report']) }}"
                aria-haspopup="true" data-menu-toggle="hover">
                <a href="javascript:;" class="menu-link menu-toggle">
        ...
        @if (in_array($user_type, ['admin']) || in_array('1106', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.transactions.index', 'admin.transactions.create']) }}"
                aria-haspopup="true" data-menu-toggle="hover">
                <a href="javascript:;" class="menu-link menu-toggle">
                ...
                <div class="menu-submenu">
                    ...
                    <ul class="menu-subnav">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.transactions.index']) }}"
                            aria-haspopup="true">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.transactions.create']) }}"
                            aria-haspopup="true">
...
@if ($addon != null)
    @if ($addon->activated)
        @if ($user_type == 'admin' || in_array('1105', json_decode(Auth::user()->staff->role->permissions ?? '[]')))
            <li class="menu-item menu-item-submenu {{ areActiveRoutes(['admin.shipments.barcode.scanner', 'admin.deliveryTime.index', 'admin.packages.index', 'admin.shipments.settings', 'admin.shipments.covered_countries', 'admin.areas.index', 'admin.areas.create', 'admin.shipments.settings.fees']) }}"
                aria-haspopup="true" data-menu-toggle="hover">
                <a href="javascript:;" class="menu-link menu-toggle">
                ...
                <div class="menu-submenu">
                    ...
                    <ul class="menu-subnav">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.deliveryTime.index']) }}"
                            aria-haspopup="true">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.shipments.covered_countries']) }}"
                            aria-haspopup="true">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.areas.index', 'admin.areas.create']) }}"
                            aria-haspopup="true">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.shipments.settings.fees']) }}"
                            aria-haspopup="true">
                        ...
                        <li class="menu-item {{ areActiveRoutes(['admin.shipments.settings']) }}"
                            aria-haspopup="true">
