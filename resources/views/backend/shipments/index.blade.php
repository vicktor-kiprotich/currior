                            ...
                            <ul class="navi flex-column navi-hover py-2">
                                ...
                                <li class="navi-item">
                                    ...
                                    @foreach ($actions as $action)
                                        @if (in_array($auth_user->user_type, $action['user_role']) ||
                                                in_array($action['permissions'] ?? '', json_decode($auth_user->staff->role->permissions ?? '[]')))
                                            @if ($action['index'] == true)
                                                @php
                                                    $action_counter++;
                                                @endphp
                                                <a href="#"
                                                    class="action_checker navi-link @if (!isset($action['js_function_caller'])) action-caller @endif"
                                                    @if (isset($action['js_function_caller'])) onclick="{{ $action['js_function_caller'] }}" @endif
                    ...
                    <div class="row align-items-center">
                        ...
                        <div class="col-lg-12 col-xl-12">
                            <div class="row align-items-center">
                                <div class="col-md-4 my-md-0 my-2">
                                    <div class="input-icon">
                                        <input type="text" name="code" value="<?php if (isset($_GET['code'])) {
                                            ...
                                            class="form-control" placeholder="{{ translate('Shipment Code') }}"
                                ...
                                @if (false)
                                    <div class="col-md-4 my-md-0 my-2">
                                        <div class="d-flex align-items-center">
                                            <label class="d-none d-md-block mb-0 mr-3">{{ translate('Customer') }}:</label>
                                            <select name="client_id" class="form-control" id="kt_datatable_search_status">
                                                <option value="">{{ translate('All') }}</option>
                                                @foreach (\App\Client::all() as $client)
                                                    <option @if (isset($_GET['client_id']) && $_GET['client_id'] == $client->id) selected @endif
                                ...
                                <div class="col-md-4 my-md-0 my-2">
                                    <div class="d-flex align-items-center">
                                        <label class="d-none d-md-block mb-0 mr-3">{{ translate('Type') }}:</label>
                                ...
                                @if ($auth_user->user_type != 'branch')
                                    <div class="col-md-4 my-md-5 my-2">
                                        <div class="d-flex align-items-center">
                                            <label class="d-none d-md-block mb-0 mr-3">{{ translate('Branch') }}:</label>
                                            <select name="branch_id" class="form-control" id="kt_datatable_search_type">
                                                <option value="">{{ translate('All') }}</option>
                                                @foreach (\App\Branch::all() as $Branch)
                                                    <option @if (isset($_GET['branch_id']) && $_GET['branch_id'] == $Branch->id) selected @endif
                        ...
                        <div class="col-lg-3 col-xl-4 mt-lg-0 mt-5">
                            <button type="submit"
                                class="btn btn-light-primary font-weight-bold px-6">{{ translate('Search') }}</button>
                            <button id="reset_search"
                                class="btn btn-light-primary font-weight-bold px-6">{{ translate('Reset') }}</button>
                        </div>
            ...
            <table class="aiz-table mb-0 table">
                <thead>
                    <tr>
                        ...
                        @endif
                        @if (
                            $status == \App\Shipment::APPROVED_STATUS ||
                                $status == \App\Shipment::CAPTAIN_ASSIGNED_STATUS ||
                                $status == \App\Shipment::RECIVED_STATUS)
                ...
                <tbody>
                    ...
                    @foreach ($shipments as $key => $shipment)
                        @if ($client_id != $shipment->client_id)
                            <tr class="bg-light">
                                ...
                                <th colspan="4">
                                    ...
                                    @else
                                        {{ is_object($shipment->client) ? $shipment->client->name : $shipment->client_id }}
                                    @endif
                                </th>
                            </tr>
                            @php
                        ...
                        <tr>
                            <td>
                                ...
                                @else
                                    <label class="checkbox checkbox-success"><input
                                            ...
                                            data-clientarea="{{ $shipment->to_area->name ?? '' }}" {{-- data-clientid="{{ $shipment->client->id }}" --}}
                                            data-clientid="{{ $shipment->client_id }}"
                            ...
                            @else
                                <td width="3%">
                                    {{ $key + 1 + ($shipments->currentPage() - 1) * $shipments->perPage() }}
                                </td>
                            ...
                            @if (in_array($user_type, ['admin', 'customer']) ||
                                    ...
                                    in_array('1006', $staff_permission))
                                <td><a
                            ...
                            @if ($status == \App\Shipment::CAPTAIN_ASSIGNED_STATUS || $status == \App\Shipment::RECIVED_STATUS)
                                <td><a href="{{ route('admin.captains.show', $shipment->captain_id) }}">
                                        @isset($shipment->captain_id)
                                            {{ $shipment->captain->name }}
                                        @endisset
                                    </a></td>
                            @endif
                            @if (
                                $status == \App\Shipment::APPROVED_STATUS ||
                                    ...
                                    $status == \App\Shipment::RECIVED_STATUS)
                                <td>
                                    @isset($shipment->current_mission->id)
                                        <a href="{{ route('admin.missions.show', $shipment->current_mission->id) }}">
                                            {{ $shipment->current_mission->code }}</a>
                                    @endisset
                                </td>
                            @endif
                            ...
                            @if ($status != 'all')
                                <td class="text-center">
                                    ...
                                    @if (
                                        $status != \App\Shipment::APPROVED_STATUS &&
                                            $status != \App\Shipment::CAPTAIN_ASSIGNED_STATUS &&
                                            $status != \App\Shipment::CLOSED_STATUS &&
                                            $status != \App\Shipment::RECIVED_STATUS &&
                                            $status != \App\Shipment::IN_STOCK_STATUS &&
                                            $status != \App\Shipment::DELIVERED_STATUS &&
                                            $status != \App\Shipment::SUPPLIED_STATUS &&
                                            $status != \App\Shipment::RETURNED_STATUS)
                                        <a class="btn btn-soft-primary btn-icon btn-circle btn-sm"
                    ...
                    <div class="modal-content">
                        @if (isset($status))
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input type="text" disabled name="Mission[client_id]" value=""
                                                    id="pick_up_client_id_hidden" class="form-control">
                                                {{-- <select value={} class="form-control" id="pick_up_client_id" disabled>
                                                    @foreach (\App\Client::all() as $client)
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <select style="background:#f3f6f9;color:#3f4254;"
                                                    name="Mission[client_id]" class="form-control" id="pick_up_client_id"
                                                    disabled>
                                                    @foreach (\App\Client::all() as $client)
                                        ...
                                        <div class="col-md-12">
                                            <div class="form-group">
                                                ...
                                                <input style="background:#f3f6f9;color:#3f4254;" type="text"
                                                    name="Mission[name]" class="form-control" id="delivery_name"
                                                    disabled />
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" name="Mission[state]" class="form-control"
                                                    id="delivery_state_hidden" />
                                                <input style="background:#f3f6f9;color:#3f4254;" type="text"
                                                    class="form-control" id="delivery_state" disabled />
                                            </div>
                                        </div>
                                        <div class="col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="hidden" name="Mission[area]" class="form-control"
                                                    id="delivery_state_hidden" />
                                                <input style="background:#f3f6f9;color:#3f4254;" type="text"
                                                    class="form-control" id="delivery_area_hidden" disabled />
                                            </div>
                                        </div>
                            ...
                            <div class="modal-footer">
                                ...
                                <button type="submit"
                                    class="btn btn-primary">{{ translate('Create Mission') }}</button>
                            </div>
                    </div>
                </div>
            </div><!-- /.modal -->
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input style="background:#f3f6f9;color:#3f4254;" id="from_branch_transfer"
                                            ...
                                            value="{{ translate('Transfer') }}" disabled="disabled" readonly />
                                    </div>
                                </div>
                                <div class="col-md-12">
                                    <div class="form-group">
